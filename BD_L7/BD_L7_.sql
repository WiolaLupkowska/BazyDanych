...
as trafione_do_stracone;
   ...
   -- c
	SELECT @x:=(select count(gole_gosp) from mecze WHERE gospodarz_id= @kid and gole_gosp>gole_gosc)
   + (select count(gole_gosp) from mecze WHERE gosc_id= @kid and gole_gosp<gole_gosc) as wygrane ,
   @y:= (select count(gole_gosp) from mecze WHERE gospodarz_id= @kid and gole_gosp=gole_gosc)   + (select count(gole_gosp) from mecze WHERE gosc_id= @kid and gole_gosp<gole_gosc) as remisy,
   @z:= @x*3+@y*1 as punkty;
...
order by m.data;
 ...
 -- f 
 select @x:=(select count(mecz_ID) from mecze where (gospodarz_id=@kid and (gole_do_przerwy_gospodarz<gole_do_przerwy_gosc) and(gole_gosp>=gole_gosc))) as moja_drużyna_gospodarzem,
 @y:= (select count(mecz_ID) from mecze where (gosc_id=@kid and (gole_do_przerwy_gosc<gole_do_przerwy_gospodarz) and(gole_gosc>=gole_gosp))) as moja_druzyna_gosciem,
 @z:=@x+@y;

-- g
select @x:=((select sum(strzaly_gospodarz) from mecze where gospodarz_id=@kid) + (select sum(strzaly_gosc) from mecze where gosc_id=@kid)) as strzaly,
@y:=((select sum(strzaly_celne_gospodarz) from mecze where gospodarz_id=@kid) + (select sum(strzaly_celne_gosc) from mecze where gosc_id=@kid)) as celne,
...
-- i
 select (select count(mecz_ID) from mecze where gosc_id=@kid and faule_gosc>faule_gospodarz)+(select count(mecz_ID) from mecze where gospodarz_id=@kid and faule_gospodarz>faule_gosc) as bardziejBrutalneMecze;
